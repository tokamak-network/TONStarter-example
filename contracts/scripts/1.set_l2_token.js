...
//abis
import ERC20AJson from "./abis/ERC20A.json";
import L1ProjectManagerProxyJson from "./abis/goerli/L1ProjectManagerProxy.json";
...
async function main() {
  ...
  await setup();
  const deployedL1 = await deployedContracts(
    L1Contracts.names,
    L1Contracts.abis,
    l1Signer
  );
  const deployedL2 = await deployedContracts(
    L2Contracts.names,
    L2Contracts.abis,
    l2Signer
  );
  ...
  let projects = await deployedL1.L1ProjectManager.projects(
    projectInfo.projectId
  );
  let l2Token = await deployedL2.L2ProjectManager.tokenMaps(
    projectInfo.l1Token
  );
  ...
  if (
    projects.l2Token == "0x0000000000000000000000000000000000000000" &&
    l2Token == "0x0000000000000000000000000000000000000000"
  ) {
    /// L2 : create L2 token
    const topic = deployedL2.L2TokenFactory.interface.getEventTopic(
      "StandardL2TokenCreated"
    );
    const receipt = await (
      await deployedL2.L2TokenFactory.createL2Token(
    ...
    /// L1 : set L2 token
    const topic1 =
      deployedL1.L1ProjectManager.interface.getEventTopic("SetL2Token");
    const receipt1 = await (
      await deployedL1.L1ProjectManager.setL2Token(
        projectInfo.projectId,
        projectInfo.l2Type,
