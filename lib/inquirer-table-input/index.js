...
class TableInput extends Base {
  ...
  onError(state) {
    if (this.editingMode) {
      ...
      if (!this.nextEnterWillConfirm) {
        this.render(this.opt.confirmMessage);   
        const values = this.rows.choices[0]
        const sumValues = []
        for (const key in values) {
          if (key > 0) {
            sumValues.push(values[key])
          }
        } 
        const totalSum = sumValues.reduce((sum, element) => sum + element, 0);

        if (totalSum === 100) {
          //need to put codes for aaditional conditions
        return this.nextEnterWillConfirm = true;
        }

        return this.render(
          chalk.redBright("Sum of token allocation should be equal to 100%")
          );  
  ...
  onEditPress(key) {
    ...
    //inputed value by this selected cell
